...
on_install() {
  ...
  latest_v2ray_version=`/data/data/com.termux/files/usr/bin/curl -s -I "${official_v2ray_link}/latest" | grep Location | grep -o "tag.*" | grep -o "v[0-9.]*"`
  ...
  /data/data/com.termux/files/usr/bin/curl "${download_v2ray_link}" -L -o "${download_v2ray_zip}" >&2
  if [ "$?" != "0" ] ; then
